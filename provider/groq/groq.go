...
// endpoint.
//
// Chat-only: Groq's OpenAI-compatible surface covers chat, models and audio, but
// publishes no /embeddings route and serves no embedding models, so
// SupportsEmbeddings reports false rather than claiming a route the caller would
// only discover is missing by spending a call. See compat.ChatOnly — if Groq
