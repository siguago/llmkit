...
1.0 的含义是**契约完成**，不是功能完成：统一 Chat / 媒体能力面、两套显式原生协议面（OpenAI Responses、Anthropic Messages）、三块资源面（OpenAI Files、OpenAI Batch、Anthropic Message Batches）的导出 API 与文档化行为自此冻结，1.x 不再破坏。没做的仍然没做，见 README 的「不做什么 · 1.x 路线」。

### 新增

- **[STABILITY.md](STABILITY.md)**：冻结范围（全部非 internal 包的导出 API + 文档化行为：`Supports*` 语义、重试语义、错误分类、wire 行为）、「破坏」的确切边界、依赖与工具链政策、废弃流程，以及每条承诺对应的机械守卫。
  - 其中最该先读的一条：**给 struct 加字段不算破坏**，请始终带字段名构造本项目的 struct。v0.6.0 给 `provider.Usage` 加缓存字段时，不带字段名的 composite literal 编译失败过 —— 那不是回归，是位置式 literal 本就不在承诺内。
  - **上游 beta 不进冻结面**：Anthropic Files 因此不在 1.0 内，待 GA 后 1.x 以纯加法接入。用 `WithBetas(...)` 给单次调用传 beta header 仍是调用方的显式选择。
- **CI `api-compat` job**：用 `apidiff` 对比最近的 release tag 的导出面，基线为 v1 及以上时出现不兼容变更即失败；基线还是 0.x 时同样比对但只报告 —— 0.x 本来就会计划性地破坏，一个每次都要被静音的门禁，到真正需要它时已经没人信了。工具经 `go run` 以钉死的版本执行，不进 `go.mod`，零依赖门禁不受影响。两条路径都做过演练：0.x 基线放行、v1 基线对本轮的两处删除如期变红。
...
| 版本 | 需要动手的地方 |
|---|---|
| [v0.7.0](#v070--2026-08-18) | 给流式调用的 context 加 deadline（900 秒兜底移除）；`SupportsImages()` / `SupportsVideo()` 改名 |
| [v0.6.0](#v060--2026-08-09) | 不带字段名构造 `provider.Usage{...}` 改为带字段名 |
| [v0.5.0](#v050--2026-08-02) | Anthropic 流式调用的 context 加 deadline |
| [v0.3.0](#v030--2026-07-31) | 显式写出 `siliconflow.New` 返回类型的代码改用 `:=` |
| [v0.2.0](#v020--2026-07-30) | 媒体重试、流式严格模式、能力探测拆细、几处删除，见该版迁移表 |
